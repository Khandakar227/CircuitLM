...
def batch_process_zeroshot(model_id=Config.DEFAULT_LLM, resume_file=None, category=None):
    generator = ZeroShotCircuitGenerator(model_id=model_id)
    results = {}
    ...
    if resume_file and os.path.exists(resume_file):
        ...
        try:
            ...
            output_file = f"output/batch_results_{model_id}{cat_str}_zeroshot_{timestamp}.json"
    else:
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        cat_str = f"_{category}" if category else ""
        output_file = f"output/batch_results_{model_id}{cat_str}_zeroshot_{timestamp}.json"
    ...
    print(f"🚀 Starting zero-shot batch processing of {len(prompts_to_process)} prompts using model: {model_id}...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Batch process circuit designs (zero-shot).")
    ...
    print(f"\n🚀 Starting zero-shot batch processing...")
    print(f"   MODEL: {args.model}")
    print(f"   CATEGORY: {args.category or 'all'}")
    print(f"   PROVIDER: {Config.LLM_PROVIDER}")
    
    batch_process_zeroshot(model_id=args.model, resume_file=args.resume, category=args.category)
